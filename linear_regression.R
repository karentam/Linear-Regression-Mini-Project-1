...
sts.ex.energy <- subset(states.data, select = c("metro", "energy"))
summary(sts.ex.energy)
# correlation between metro and energy
cor(na.omit(sts.ex.energy))

# scatter plot of metro vs energy
plot(na.omit(sts.ex.energy))
...
# Metro is significant to the model. But the model has an adjusted R-squared of 0.097, 
...
## Two more predictors are added: income and density

# 1. Examine/plot the data before fitting the model

# summary of energy, metro, income, and density columns, all rows
sts.ex.energy2 <- subset(states.data, select = c("metro", "energy", "income", "density"))
summary(sts.ex.energy2)
# correlation between metro, income, density, and energy
cor(na.omit(sts.ex.energy2))

# scatter plot of metro, income, density, and energy
plot(na.omit(sts.ex.energy2))

##   2. Print and interpret the model `summary'

# Fit our regression model
energy.mod2 <- lm(energy ~ metro + density + income, # regression formula
               data=states.data) # data set
# Summarize and print the results
...
class(energy.mod2)
names(energy.mod2)
methods(class = class(energy.mod2))[1:9]
...
## but no predictor is significant to the model. 

## The percentage error of the model with metro is 27.98%. 
...
#Add the interaction to the model
energy.metro.by.income <- lm(energy ~ metro*income,
                             data=states.data) 
#Show the results
coef(summary(energy.metro.by.income)) # show regression coefficients table
